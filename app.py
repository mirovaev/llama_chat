...
def chat():
    ...
    # Проверяем, завершена ли сессия заказа
    ...
    # Если AI подтвердила заказ, завершаем сессию
    if "доставка" in reply.lower() or "ваш заказ" in reply.lower():
        session["completed"] = True  # Отмечаем завершённый заказ

    # Если AI сгенерировала заказ, отправляем его в Telegram
    if any(phrase in reply.lower() for phrase in ["ваш заказ", "доставка", "оформлен"]):
